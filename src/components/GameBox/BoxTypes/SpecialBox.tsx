...
export const SpecialBox = ({ type, name, price, pricetext }) => {
  const getBoxImage = () => {
    if (type === BOX_TYPES.CHANCE) return <img src={chanceIcon} alt={`${type} Icon`} />
    if (type === BOX_TYPES.COMMUNITY) return <img src={communityIcon} alt={`${type} Icon`} />
    if (type === BOX_TYPES.RAILROADS) return <img src={railRoadIcon} alt={`${type} Icon`} />
    if (type === BOX_TYPES.TAX) return <img src={taxIcon} alt={`${type} Icon`} />
  }
